...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("nanocats.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "nanocats.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("nanocats.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "nanocats.channels.matrix.asyncio.create_task", _fake_create_task
    )
