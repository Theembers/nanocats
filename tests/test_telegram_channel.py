...
from nanocats.channels.telegram import TELEGRAM_REPLY_CONTEXT_MAX_LEN, TelegramChannel
from nanocats.channels.telegram import TelegramConfig
...
class _FakeBot:
    ...
    async def get_me(self):
        self.get_me_calls += 1
        return SimpleNamespace(id=999, username="nanocats_test")
...
async def test_start_uses_request_proxy_without_builder_proxy(monkeypatch) -> None:
    ...
    monkeypatch.setattr("nanocats.channels.telegram.HTTPXRequest", _FakeHTTPXRequest)
    monkeypatch.setattr(
        "nanocats.channels.telegram.Application",
...
async def test_group_policy_mention_accepts_text_mention_and_caches_bot_identity() -> None:
    ...
    await channel._on_message(_make_telegram_update(text="@nanocats_test hi", entities=[mention]), None)
    await channel._on_message(_make_telegram_update(text="@nanocats_test again", entities=[mention]), None)
...
async def test_group_policy_mention_accepts_caption_mention() -> None:
    ...
    await channel._on_message(
        _make_telegram_update(caption="@nanocats_test photo", caption_entities=[mention]),
        None,
    )

    assert len(handled) == 1
    assert handled[0]["content"] == "@nanocats_test photo"
...
async def test_download_message_media_returns_path_when_download_succeeds(
    ...
    monkeypatch.setattr(
        "nanocats.channels.telegram.get_media_dir",
