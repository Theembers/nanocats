...
from nanocats.agent.tools.web import WebSearchTool
from nanocats.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    ...
    result = await tool.execute(query="nanocats", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("nanocats.agent.tools.web.DDGS", MockDDGS, raising=False)
    import nanocats.agent.tools.web as web_mod
