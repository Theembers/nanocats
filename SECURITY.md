...
If you discover a security vulnerability in nanocats, please report it by:
...
chmod 600 ~/.nanocats/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.nanocats/config.json` with file permissions set to `0600`
...
- ✅ Run nanocats with a dedicated user account
...
- Keep authentication data in `~/.nanocats/whatsapp-auth` secure (mode 0700)
...
pip install --upgrade nanocats-ai
```
...
- Subscribe to security advisories for nanocats and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash nanocats
   sudo -u nanocats nanocats gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.nanocats
   chmod 600 ~/.nanocats/config.json
   chmod 700 ~/.nanocats/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.nanocats/logs/nanocats.log
   ```
...
- **Chat history is stored locally** - protect the `~/.nanocats` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.nanocats/logs/nanocats.log
   ```
